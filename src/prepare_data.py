...
def create_dataset(
    ...
    # Load dataset
    if dataset == "wikihow":
        hf_dataset = load_dataset(*DATASET_NAME_MAPPER[dataset], data_dir="./wikihow_data/")
    else:
        hf_dataset = load_dataset(*DATASET_NAME_MAPPER[dataset])

    # Load tokenizer
    tokenizer = AutoTokenizer.from_pretrained(tokenizer)

    # Handle split naming differences (e.g., val vs validation)
    split_aliases = {
        "train": ["train"],
    ...
    for split_out, num_sample in [("validation", sample_val), ("train", sample_train), ("test", sample_test)]:
        split_in = next((k for k in split_aliases[split_out] if k in hf_dataset), None)
        if split_in is None:
            continue
        ...
        out_path = pathlib.Path(output_dir).joinpath(f"{split_out}.jsonl")
        with jsonlines.open(out_path, "a" if append_mode else "w") as f:
            f.write_all(processed_split)

        plot_length_info(
            [item["input_length_info"] for item in processed_split],
            str(pathlib.Path(output_dir).joinpath(f"{dataset_name}_{split_out}_input_length.png")),
        )
        plot_length_info(
            [item["output_length_info"] for item in processed_split],
            str(pathlib.Path(output_dir).joinpath(f"{dataset_name}_{split_out}_output_length.png")),
        )
...
def main():
    ...
    if args["dataset"] in ["all", "all_other"]:
        mode = args["dataset"]
        del args["dataset"]
        for ds_name in DATASET_NAME_MAPPER.keys():
            if mode == "all_other" and ds_name in ["cnn", "arxiv", "pubmed"]:
                continue
            create_dataset(dataset=ds_name, **args, append_mode=True)
    else:
